...
abundant :: Int -> Bool 
abundant n = sum [el \\ el<-[1..(n/2)] | n rem el == 0 ] > n
...
fact :: Int -> Int
fact n = prod [el \\ el <- [1..n]]

strong_num :: Int -> Bool
strong_num n = sum [fact el \\ el<- toList n] == n
...
filtered n lst = [n ^ i \\ el<-lst & i<-[0..((length lst)-1)] | el rem n == 0]
...
retrieve lst n = [(el,x) \\ el <-(z) , x <-(z) | (abs(el - x) == n)]
where z = sort (removeDup lst)
...
find_max_index lst = index (maxList lst) lst
...
helper [] = -1.0
helper lst
| (length lst) rem 2 == 0 = (toReal (lst !! (z-1)) + toReal (lst !! (z))) / 2.0
= toReal (lst !! z)
where z = (length lst)/2

median :: [Int] -> Real
median lst = helper (sort lst)
...
find_prime_sum_pairs lst lst1 = [(x,y) \\ x <- lst , y <- lst1 | isPrime (x+y) ]
...
inc3 [x] = [x]
inc3 [] = []
inc3 [x,y,z:xs] = [x,y,x+y+z: inc3 xs]
...
help fun n = [f n \\ f <- fun ] 


mapFunList :: [Int] -> [[Int]]
mapFunList lst = [help funList el \\ el <- lst]
...
name :: Int [(String,Int,Int)] -> String
name n [] = ""
name n [x:xs]
| (n == thd3 x) = fst3 x
= name n xs
...
highestSval lst = name (maxList [thd3 tuple \\ tuple <- lst]) lst
...
checkA :: [Int] Int -> [Int]
checkA lst n = [el \\el <- (dropWhile ((<>) n) lst)  | isEven  el]
...
doubleUntilNeg lst = [el*2 \\ el <- takeWhile ((<>)z) lst] ++ [el \\ el <- checkA lst z | isEven el]
where z = find lst
...
got :: a [(a,Int)] -> (a,Int) | Eq a
got n lst = hd [(n,snd tuple) \\ tuple <- lst | n == (fst tuple)]


do :: a [(a,Int)] [a] -> (a,Int) | Eq a
do n tuple clst
| isMember n clst = got n tuple
= (n,-1)


elementLocation :: [a] [a] -> [(a,Int)] | Eq a
elementLocation ilst mlst = [do el ([(el,i) \\ el <- ilst & i <- [0..]]) ilst  \\ el <- mlst]
