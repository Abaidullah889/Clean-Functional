...
isDisariumNum n = (sum [(el^i) \\ el <- (toList n) & i <- [1..(length (toList n))]]) == n
...
harshadNums :: [Int] -> [Int]
harshadNums lst = [el \\ el <- lst | (el rem (sum (toList el))) == 0]
...
isLeader n lst = (length [el \\ el <- lst | n < el]) == 0 
...
filteredRem n lst = [(el rem n) \\ el <- lst | (el rem n <> 0)]
...
countGoodNums lst = length [el \\ el <- lst | (el >= 10 && el <= 99 && el rem 3 == 0)]
...
validTriangles tuple = [(check el) \\ el <- tuple]
...
hello tuple = [(fst tuple) \\ i <- [1..(snd tuple)]]

stringCopy :: [(String,Int)] -> [[String]]
stringCopy tuple = [(hello el) \\ el <- tuple]
...
concat [x] [y] = ((toString x) +++ (toString y))
concat [x:xs] [y:ys] = ((toString x) +++ (toString y)) +++ (concat xs ys) 


intInsertion :: Int Int -> Int
intInsertion l r = toInt (concat (toList l) (toList r))
...
group_by_marks lst pm = ([el \\ el <- lst | ((snd el) <= 30)] , [el \\ el <- lst | ((snd el) > 30)])
...
vowFreq :: [Char] -> Int
vowFreq lst = length [el \\ el <- lst | el == 'a' || el == 'e' || el == 'i' || el == 'o' || el == 'u']


cipherList :: [Char] -> [Char]
cipherList lst = [(toChar ((toInt el) + (vowFreq lst))) \\ el <- lst]
...
Reach :: Int Int Int Int Int -> Bool
Reach x x1 y y1 n
| (x == y) && (x1 == y1) = True
|((x > y) && (x1 > y1)) = False
| isOdd(n) = (Reach x (Odd x x1 y1) y y1 (n+1))
| isEven(n) = (Reach (Even x x1 y) x1 y y1 (n+1))  


isReachable :: (Int,Int) (Int,Int) -> Bool
isReachable tuple tuple2 = (Reach (fst tuple) (snd tuple) (fst tuple2) (snd tuple2) 1)
...
evaluate lst n = sum [(el * (n^i)) \\ el <- lst & i <- [0..((length lst)-1)]]
...
Mover lst n t = (index n (toList t)) - (index n (toList lst))
