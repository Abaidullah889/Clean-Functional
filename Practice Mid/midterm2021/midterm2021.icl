...
tuples_with_property tuple = [helper el \\ el <- tuple] 
...
maker :: Int -> [Int]
maker n = [el \\ el <- [n,n-1..1]] ++ [el \\ el<-[1..n]]


decreasing_list :: Int -> [[Int]]
decreasing_list n = [maker el \\ el <- [n,n-1..1]] ++ [maker el \\ el <- [1..n]]
...
matchIndex lst f = and [f (length el) == f i \\ el<-lst & i<-[1..length(lst)]]
...
max_even_sum lst
| length [sum el \\ el <- lst | isEven (sum el)] == 0 = -1
= maxList [sum el \\ el <- lst | isEven (sum el)]
...
isPalind :: Int -> Bool
isPalind n = (list n) == reverse (list n)

isPrime :: Int -> Bool
isPrime lst = length [el \\ el <- [1..lst] | lst rem el == 0] == 2 

fPrime :: [Int] -> [Int]  
fPrime lst = [el \\ el <- lst | isPrime el]

fPalind :: [Int] -> [Int]
fPalind lst = [el \\ el <- lst | isPalind el]

extract :: [[Int]] -> [(Int, Int)]
extract [] = []
extract [x:xs] = (zip2 (fPrime x) (fPalind x))  ++ (extract xs)				
...
primeFactors n = [el \\ el <- [2..n] | ((n rem el) == 0 && isPrime el)] 
...
coPrime n m = length [el \\ el <- [1..n] | (n rem el ==0 && m rem el == 0)] == 1

phi :: Int -> Int
phi n = length [el \\ el <- [1..n] | coPrime el n]
...
pFactors :: Int -> [Int]
pFactors n = [el \\ el <- [1..n] | (n rem el == 0) && isPrime el]

multicity :: Int Int -> Int
multicity n t
| (length [el \\ el<-[1..n] | t rem (n^el) == 0] == 0) = 1
= maxList [el \\ el<-[1..n] | t rem (n^el) == 0]

prime_factors_mult :: Int -> [(Int,Int)]
prime_factors_mult n = [(el,(multicity el n)) \\ el <- (pFactors n)]
...
intersection lst n = [el \\ el <- lst | (fst el) <= (snd n) && (snd el)  >= (fst n)]
...
tuple [] = []
tuple [x,y,z:xs] = [(x,y,z)] ++ (tuple xs)


tuple_sort :: [(Int,Int,Int)] -> [(Int,Int,Int)]
tuple_sort lst =  tuple (sortBy (<) ([fst3 el \\ el<-lst] ++ [snd3 el \\ el<-lst] ++ [thd3 el \\ el<-lst]))
