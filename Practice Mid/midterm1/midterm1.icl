...
append :: [[Int]] -> [[Int]]
append lst = map (\x  =  x ++ [sum x] ++ [length x]) lst 
...
fraction :: [Real] -> [Real]
fraction lst = [el - (helper el) \\ el <- lst ]
...
freq :: Int [Int] -> Int
freq x lst = length [el \\ el <- lst | el == x ]

famousNum :: [Int] -> [Int]
famousNum lst = [el \\ el <- lst | (freq el lst) >= 5]
...
findPrev n lst = getValue lst ((find n lst)-1)  
...
symmetricDif lst1 lst2 = removeDup ([el \\ el <- lst1 | (isMember el lst2) == False] ++ [el \\ el <- lst2 | (isMember el lst1) == False])
...
notN :: Int [Int] -> [Int]
notN n lst = map (\x = x ^ 4)(takeWhile ((<>) n) lst) 
...
isGood :: [Int] [Int] -> Bool
isGood lst uni = length [el \\ el <- lst | isMember el uni] >= ((length lst) / 2)


goodLists :: [[Int]] [Int] -> Int
goodLists lst uni = length [el \\ el <- lst | (isGood el uni)]
...
coPrimes l r = isFreq2 z z == False
where z = ([el \\ el <-[2..(l-1)] | l rem el == 0] ++ ([el \\ el <- [2..(r-1)] | r rem el == 0]))
...
cleanN :: Int Int Int Int -> Int
cleanN 0 a b c = a
cleanN 1 a b c = b 
cleanN 2 a b c = c 
cleanN n a b c = (((cleanN (n-1) a b c ) * (cleanN (n-2) a b c)) +(cleanN (n-3) a b c)) rem 1000



Start = cleanN 3 1 2 3

clean :: Int Int Int Int -> [Int]
clean 1 a b c = [(cleanN 0 a b c)]
clean n a b c = [cleanN (n-1) a b c] ++ (clean (n-1) a b c)
