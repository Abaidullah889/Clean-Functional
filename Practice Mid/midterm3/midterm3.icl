...
cond21 :: Int -> Bool
cond21 x = length (filter ((==)1) (toList x)) >= 2


doubleOne :: [Int] -> [Int]
doubleOne x = [el \\ el <- x | cond21 el] 
...
diff :: [Int] [Int] -> [Int]
diff lst x = [el \\ el <- lst | (isMember el x) == False]

difference :: [[Int]] [[Int]] -> [[Int]] 
difference lst x = [diff a b \\ a <- lst & b <- x]
...
rep :: [Int] Int -> [Int]
rep [] x = [x]
rep lst x = updateAt ((length lst) / 2) x lst

repMid :: [[Int]] Int -> [[Int]]
repMid lst x = [rep el x \\ el <- lst]
...
isPrime :: Int -> Bool
isPrime x = length [el \\ el <- [1..x] | x rem el == 0] == 2


primes7 :: [Int] -> [Int]
primes7 x = [el \\ el <- x | (isPrime el) && ((el rem 10) == 7)] 
...
evenOld :: (Int,Int) -> Bool
evenOld tuple = isEven (fst tuple) && isOdd (snd tuple)

holdsTrue :: [(Int, Int)] -> Bool
holdsTrue [] = False
holdsTrue lst = and [evenOld el \\ el <- lst]
...
digitSum x
| x < 10 = x
= digitSum (sum (toList x))


super_digit :: [Int] -> [Int]
super_digit lst = [digitSum el \\ el <- lst]
...
isPower c p = length [el \\ el <- [0..c] | p^el == c] == 1


powersList :: [Int] Int -> [Int]
powersList lst x = [el \\ el <- lst | isPower el x]
...
twinPrimes l r = length [(a,b) \\ a <-[l..r] , b <- [l..r] | (isPrime a && isPrime b) && ((a-b) == 2)]
