...
getBestBScStudent lst = (hd [el \\ el <- lst | (sum (el.grades)) / ((length el.grades)) == max]).id 
where max = maxList [(sum (el.grades))/(length (el.grades))  \\ el <- lst]
...
freq c str = length [el \\ el <- [el \\ el <-: str] | c == el]


count :: String -> {(Char, Int)}
count str = {(a,b) \\ (a,b) <- [(el,freq el str) \\ el <- (removeDup [el \\ el <-: str])] | b >= 2} 
...
instance - [Int] where
	(-) lst lst1 = [b \\ b <- lst | not (isMember b lst1)]
...
Insert (Pointer str list) s = (Pointer str (Insert list s))
...
gether :: (LinkedList String) -> [String]
gether Nil = []
gether (Pointer s list) = [s] ++ gether list

putter :: (LinkedList String) [String] -> (LinkedList String)
putter Nil [] = Nil
putter (Pointer s list) [x:xs] = (Pointer x (putter list xs))

Reverse :: (LinkedList String) -> (LinkedList String)
Reverse tree  = putter tree (reverse (gether tree))
...
delete Nil str = Nil
delete (Pointer s list) str
| str == s  = delete (list) str
= (Pointer s (delete(list) str))
...
concat Nil tree1 = tree1
concat (Pointer s Nil) tree1 = (Pointer s tree1)
concat (Pointer s list) tree1 = (Pointer s (concat list tree1))
...
findFakes arr = {el \\ el <- lst}
where lst =  removeDup [el \\ el <-: arr]
...
diagonalSum :: [[Int]] -> Int
diagonalSum lst = sum [(el!!i) \\ el <- lst & i <- [0..(length lst)-1]]


largestMat :: {[[Int]]} -> [[Int]]
largestMat arr =  fst (last ( (sortBy (\(x,y) (a,b) = (y < b)) [(el, diagonalSum el) \\ el <-: arr])))
//where lst = [el \\ el <-: arr]
...
getValues :: (MyList a) -> String | toString a
getValues Empty = ""
getValues (Elem a list) = (toString a) +++ "," +++ (getValues list) 
...
firstAndMiddle Leaf = []
firstAndMiddle (Node (FirstName a) le ri) = [a] ++ (firstAndMiddle le) ++ (firstAndMiddle ri)
firstAndMiddle (Node (MiddleName a) le ri) = [a] ++ (firstAndMiddle le) ++ (firstAndMiddle ri)
firstAndMiddle (Node (LastName a) le ri) = (firstAndMiddle le) ++ (firstAndMiddle ri)
