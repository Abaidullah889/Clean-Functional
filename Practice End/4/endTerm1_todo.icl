...
where toInt per = size (per.firstName) + per.age + per.height 
...
where isEven er = isEven (er.age + er.height)
...
check :: String -> Bool
check str = isPrime (length [el \\ el <-: str | el == 'i' ])

helper :: {Country} -> Bool
helper str =  or [(check el.capital) \\ el <-: str]

continentsPrimeI :: [Continent] -> [String]
continentsPrimeI lst = [el.contName \\ el <- lst | (helper el.countries)]
...
shouldWePlay lst gk = length [el \\ el <- lst | el.skillLevel >= gk.skillLevel] >= 3
...
concat :: [Int] -> String
concat [x] = (getString x)
concat [x:xs] = (getString x) +++ "-" +++ (concat xs) 

getString :: Int -> String
getString n = numberList.[n]
...
freq :: Int [Int] -> Int
freq n lst = length [el \\ el <- lst | el == n]

 
f7 :: {Int} -> Bool
f7 arr = and [True \\ el <-:arr | (freq el lst) >= 2]
where lst = [el \\ el <-: arr]
