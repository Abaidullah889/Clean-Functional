...
isVowel :: Char -> Bool
isVowel c = isMember c vowels

removeVowel :: String -> String
removeVowel str = {el \\ el <-: str | not(isVowel el)}
...
trimWhiteSpace str =  toString (reverse (dropWhile (\x = x == ' ') (reverse (dropWhile (\x = x == ' ')[el \\ el <-: str]))))
...
nodeCountByValue (Node n le ri) x
| n == x = 1 + (nodeCountByValue le x) + (nodeCountByValue ri x)
= (nodeCountByValue le x) + (nodeCountByValue ri x)
...
t2l :: (Tree a) -> [a]
t2l Leaf = []
t2l (Node n le ri) = (t2l le) ++ [n] ++ (t2l ri)

bsTree :: (Tree a) -> Bool | Ord , Eq a
bsTree tree = ls == sort ls
where ls = t2l tree
...
countHeap Leaf = 0
countHeap (Node n le ri)
| n > (NodeExtractInt le + NodeExtractInt ri) = 1 + (countHeap le) + (countHeap ri)
= (countHeap le) + (countHeap ri)
...
isSameTree tree tree1 = (t2l tree) == (t2l tree1)
...
//subList arr lst1 = [el \\ el <-: arr & i <- [0..(size arr)] | (isMember i lst1)]
