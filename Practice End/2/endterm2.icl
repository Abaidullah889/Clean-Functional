...
-- <Abaidullah Asif, ZLWD2B> for the end-term programming retake assignment of FP.
...
shiftCipher str n = {toChar (((((toInt el) - 65) + n) rem 26)+65) \\ el <-: str}
...
distance :: Point Circle -> Bool
distance point brd = sqrt ((brd.center.x - point.x)^2.0 + (brd.center.y - point.y)^2.0) < brd.radius
 

check :: Circle {Point} -> Bool
check brd points = or [(distance el brd) \\ el <-: points]
...
instance == Book
where (==) a b = (a.title == b.title) && (a.author == b.author) && (a.pyear == b.pyear) && (a.num_of_pages == b.num_of_pages)
...
helper :: {Book} {Book} -> {Book}
helper arr arr1 = {el \\ el <- (sort (removeDup (lst ++ lst1)))}
where 
	lst = [el \\ el <-: arr]
	lst1 = [el \\ el <-: arr1]

instance + Library
where (+) a b = {lib_name = (a.lib_name +++ b.lib_name),books =(helper a.books b.books)}
...
checkBooks :: {Book} {Book} -> Bool
checkBooks arr arr1
| length lst <> length lst1 = False
 = length [el \\ el <- lst | (isMember el lst1)] == (length lst) 
where 
	lst = [el \\ el <-: arr]
	lst1 = [el \\ el <-: arr1]

instance == Library
where (==) a b = checkBooks a.books b.books
...
concat [x:xs] [] = [x] ++ concat [] xs
concat [x:xs] [a:b] = [x,a] ++ (concat xs b)

concatIni :: String String -> String
concatIni str str1 = {el \\ el <- ans}
where 
	lst = [el \\ el <-: str]
	lst1 = [el \\ el <-: str1]
	ans = concat lst lst1
...
instance + {String}
where (+) str str1 = {concatIni el el1 \\ el <-: str & el1 <-: str1}
...
depth :: (Tree Int) -> Int
depth Leaf = 0
depth (Node n le ri) = 1 + max (depth le) (depth ri)   



AVL_prop_check :: (Tree Int) -> Bool
AVL_prop_check Leaf = True
AVL_prop_check (Node n le ri)
| abs(depth le - depth ri) <= 1 = True && (AVL_prop_check le) && (AVL_prop_check ri)
=  False
...
doSum :: Int (Tree Int) -> Int
doSum n tree = (sum [el \\ el <- (toList tree) | n < el ]) + n

helper1 :: (Tree Int) (Tree Int) -> (Tree Int)
helper1 Leaf _ = Leaf
helper1 (Node n le ri) tree = (Node ((doSum n tree)+n) (helper1 le tree) (helper1 ri tree))


transform :: (Tree Int) -> (Tree Int)
transform tree = helper1 tree tree
...
tsum t = abs (fst3 t + snd3 t + thd3 t) 

unique :: {(Int,Int,Int)} -> (Int,Int,Int)
unique arr = hd [el \\ el <-: arr | ((fst3 el <> snd3 el) && (snd3 el <> thd3 el) && (thd3 el <> fst3 el)) && (tsum el) <= z]
where z = minList [tsum el \\ el <-:arr | ((fst3 el <> snd3 el) && (snd3 el <> thd3 el) && (thd3 el <> fst3 el))]

multiply :: (Int, Int, Int) (Int, Int, Int) -> (Int, Int, Int)
multiply (a,b,c) (x,y,z) = (a*x,b*y,c*z)

alterArray :: {(Int, Int, Int)} -> {(Int, Int, Int)}
alterArray arr = {multiply el z  \\ el <-: arr}
where z = unique arr

//Start = unique {(0,0,0),(1,-1,2), (-1,3,2), (-3,-1,-2)}
...
where (==) a b = a.yearOfMarriage == b.yearOfMarriage
...
concat1 :: (EngagedPerson, EngagedPerson) -> String
concat1 t = {el \\ el <- (getLast [el \\ el <-:((fst t).fullName)])}


getOldestMarriage :: {(EngagedPerson, EngagedPerson)} -> String
getOldestMarriage arr = concat1 (hd (sort lst))
where lst = [el \\ el <-: arr]
...
getIndex :: [Color] Color -> Int
getIndex clrs c = hd [i \\ el <- clrs & i <- [0..6] | el == c]

neighbours :: [Color] -> [Color]
neighbours  lst = [getColor (rainbow) ((getIndex rainbow el) + 2) \\ el <- lst]
...
/*
By finding the heights of left and right subtrees and combining them, we get the longest path that passes through the root.
By comparing the above path's length with diameters of left and right subtrees, we get the overall diameter of the binary tree.
...
bt20 = (BTNode2 BTLeaf2 BTLeaf2)

height :: BinaryTree2 -> Int
height BTLeaf2 = 1
height (BTNode2 le ri) = 1 + (max (height le) (height ri))

getBTDiameter :: BinaryTree2 -> Int
getBTDiameter BTLeaf2 = 1
getBTDiameter (BTNode2 le ri) =  ((height le) + (height ri) + 1) 
